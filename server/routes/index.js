  ...
  db.query('SELECT user_type FROM users WHERE username = ?', [sessionUsername], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      if (user.user_type === 'Premium') {
        ...
        res.send(`<script>alert('Anda tidak mengubah API Keys, harap membeli Premium terlebih dahulu!'); window.location.href = '/pages/pricing';</script>`);
      }
    } else {
      res.send(`<script>alert('User tidak ditemukan'); window.location.href = '/pages/account';</script>`);
    }
  });
});
